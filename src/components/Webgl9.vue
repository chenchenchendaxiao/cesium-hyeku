<template>
    <div>
        <canvas id="webgl9" width="300" height="300"  style="left:0px;background-color: rgb(209, 203, 200);position: absolute;z-index: 999;top:300px;"></canvas>
    </div>
</template>
<script>
export default {
    methods:{
        webGlInit() {
            var canvas=document.getElementById('webgl9');
            ...
            //关于计算一个顶点在平行光照下的颜色计算 首先计算一个平行光方向向量和顶点法向量的点积 得到一个类似光照强度的概念 dot就是计算点积的 然后顶点当这个值小于零那就是0代表光照没照到 而且rgb也没有负值 最后用环境光的颜色×这个强度*顶点颜色 \n\
            // 计算平行光方向向量和顶点法向量的点积\n\
