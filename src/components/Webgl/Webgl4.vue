<template>
    <div>
        <canvas id="webg4" width="300" height="300"  style="left: 900px;background-color: rgb(17, 185, 59);position: absolute;z-index: 999;top:0;"></canvas>
    </div>
</template>
<script>
export default {
    methods:{
        webGlInit() {
            var canvas=document.getElementById('webg4');
            var gl=canvas.getContext('webgl');
            //给内置变量gl_PointSize赋值像素大小 attribute声明vec4类型变量apos
            //attribute 关键字 声明顶点相关的数据的时候就会用到 生声明了之后就可以让这个变量从JavaScript中去读取数据
            //gl_Position =vec4(apos.x-0.4,apos.y,apos.z,1); 这可以让每个顶点在x轴方向偏移0.4
            //第二种平移方法 用以个平移矩阵 引入了新的数据类型 mat4 代表一个4x4的矩阵 用这个平移矩阵乘上原来的列向量vec4（这代表每个顶点）
            //创建平移矩阵(沿x轴平移-0.4)
            //1   0   0  -0.4
            //0   1   0    0
            //0   0   1    0
