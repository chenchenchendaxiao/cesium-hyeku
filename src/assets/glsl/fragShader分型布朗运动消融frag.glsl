...
float fbm(vec3 p){
    //value代表结果值、amplitude代表振幅、frequency代表频率，lacunarity代表空隙、persistance代表持续度、scale代表缩放程度、octaves代表音度。
    float value=0.;
    float amplitude=1.;
    float frequency=1.;
    float lacunarity=2.;
...
vec4 dissolve(vec2 uv){
    ...
    float edgeWidth=.04;
    vec3 edgeColor=vec3(0.0627, 0.902, 0.5098);
    ...
    float pr=smoothstep(progress-.01,progress,noise);//边界值1 边界值2 对比值 noise小于当前鼠标x坐标比值progress就0，中间就平滑 大于就1
    col.rgb=mix(col.rgb,col1.rgb,1.-pr);//当pr为1的时候 也就是鼠标在最有就显示第二个 中间一点点是平滑的 左边pr是0，那就是全第一张图
